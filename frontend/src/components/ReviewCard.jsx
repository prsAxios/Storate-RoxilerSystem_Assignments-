...
const ReviewCard = ({
  ...
  useEffect(() => {
    setReplyCount((review?.comments && review.comments.length) || 0);
  ...
  const toggleLike = async () => {
    if (!!role) {
      ...
      axios
        .post(
          `${import.meta.env.VITE_BACKEND_URL}/stores/${storeId}/reviews/${review._id
          }/like`,
          {},
          {
            headers: {
          ...
          {(role === "admin" || userId != review.userId._id) && (
            <AlertDialog open={openReport} onOpenChange={setOpenReport}>
              <AlertDialogTrigger asChild>
                <Button
                  variant="outline"
                  className={`flex ${role !== "admin" && "ml-auto"
                    } py-2 px-2 gap-2`}>
                  <AlertCircle />
